...
# Both Mel and MuQ directories are now needed
...
BATCH_SIZE = 32
# Using the best performing LR and scheduler from previous experiments
LEARNING_RATE = 3e-4
EPOCHS = 50
...
class QuadFusionNet(nn.Module):
    def __init__(self, emb_dim, genre_dim, audio_feature_dim, n_classes):
        ...
        # Branch 2: MuQ Embedding (re-integrated)
        ...
        # Updated Fusion Head to accept input from all four branches
        cnn_output_dim = 256
        emb_output_dim = 128
        genre_output_dim = 32
...
class QuadFusionDataset(Dataset):
    def __init__(self, dataframe, indices, mel_dir, muq_dir, feature_cols, n_genres, genre_to_idx):
        ...
        self.muq_dir = muq_dir # Added MuQ directory
...
def pad_collate_fn(batch):
    ...
    # Use default collate for all other tensors as they are fixed-size
...
def run_experiment(run_num, full_df, feature_cols, emb_dim, n_genres, genre_to_idx, class_names, class_weights):
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weights) # Label smoothing can be added back if needed
...
if __name__ == '__main__':
    ...
    # Check for both Mel and MuQ files now
    ...
    EMB_DIM = 1024 # Define MuQ embedding dimension
    ...
    for i, name in enumerate(CLASS_NAMES): print(f"  - {name}: {class_weights[i]:.2f}")
    ...
    ax_metrics.text(0.5, 0.5, metrics_text, ha='center', va='center', fontfamily='monospace', fontsize=12, bbox=dict(boxstyle="round,pad=1", fc='lightcyan', alpha=0.8))
