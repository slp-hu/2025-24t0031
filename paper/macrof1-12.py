...
def evaluate_recommendation_list(all_pred_scores, all_true_lists, k=10):
    ...
    emo_precision_list = []
    emo_recall_list = []
    emo_f1_list = [] # 新增：用于存储每种情感的 F1 分数
    
    print(f"\n> 正在计算 12 种情感各自的 Precision@{k}, Recall@{k} 以及 F1@{k}...")
    ...
    for emo_idx in range(N_EMOTIONS):
        ...
        # F1@K 计算 (避免分母为0)
        if (p_at_k + r_at_k) > 0:
            f1_at_k = 2 * (p_at_k * r_at_k) / (p_at_k + r_at_k)
        else:
            f1_at_k = 0.0
            
        emo_precision_list.append(p_at_k)
        emo_recall_list.append(r_at_k)
        emo_f1_list.append(f1_at_k)
        
        # 打印每个情感的具体明细
        print(f"  - {emo_name:12}: P@{k}={p_at_k:.4f}, R@{k}={r_at_k:.4f}, F1@{k}={f1_at_k:.4f} (Hits: {hits}/{k})")
...
if __name__ == '__main__':
    ...
    # 【新增】计算各个流派的歌曲数量，并过滤出歌曲数 >= 150 的流派
    ...
    print(f"> 保留了 {len(valid_genres)} 个流派。")
    
    # 2. 初始化环境 (使用过滤后的 df_filtered)
    ...
    genre_list = sorted(df_filtered["major_genre_name_en"].unique()); genre_to_idx = {g: i for i, g in enumerate(genre_list)}
    ...
    # 调用针对推荐列表的评估函数，并接收返回的 macro_f1
    p_at_10, r_at_10, macro_f1_at_10 = evaluate_recommendation_list(all_scores_matrix, all_labels, k=K)
    ...
    print(f"   Macro F1@{K}       : {macro_f1_at_10:.4f}  (所有情感 F1@{K} 的宏平均)")
    print(f"{'='*50}\n")