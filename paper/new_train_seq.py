...
ES_PATIENCE = 7
BEST_MODEL_PATH = "best_ranking_model_by_ndcg.pth" # Renamed to reflect new strategy
...
class ListNetLoss(nn.Module):
    ...
    This version avoids loops and in-place operations, making it safe for backpropagation.
    """
    def __init__(self):
...
def run_experiment(run_num, full_df, feature_cols, emb_dim, n_genres, genre_to_idx):
    ...
    # 【MODIFIED】 Scheduler now monitors the nDCG score in 'max' mode.
    scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.2, patience=3, min_lr=1e-7)
    
    history = {"train_loss": [], "val_loss": [], "val_ndcg_3": []}
    # 【MODIFIED】 Changed variable from best_val_loss to best_val_ndcg and initialized to 0.
    best_val_ndcg = 0.0
    wait_for_es = 0
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 【MODIFIED】 Scheduler now steps based on the validation nDCG score.
        ...
        # 【MODIFIED】 The condition now checks if the current nDCG is the best one seen so far.
    ...
    # Load the best model based on nDCG, not the last one saved by loss
