...
from sklearn.model_selection import train_test_split  # 新增引用
...
def evaluate_recommendation_list(all_pred_scores, all_true_lists, k=10):
    ...
    for emo_idx in range(N_EMOTIONS):
        ...
        # Precision@K: 推荐的 K 首歌里对了几首
        p_at_k = hits / k
        
        # Recall@K: 全库这么多属于该情感的歌，前 K 名里抓到了几首
...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if row.major_genre_name_en in self.genre_to_idx: 
            genre_vector[self.genre_to_idx[row.major_genre_name_en]] = 1.0
...
if __name__ == '__main__':
    print("--- > Starting Data Preparation (Split Logic) ---")
    ...
    # 2. 清洗数据（必须与训练代码完全一致，否则 split 结果会对不齐）
    ...
    print(f"  - Training Set (Skipped): {len(train_df)}")
    print(f"  - Validation Set: {len(val_df)}")
    print(f"  - Test Set: {len(test_df)}")
    print(f"  - [Target] Eval Set (Val+Test): {len(eval_df)}")

    # 5. 初始化环境
    feature_cols = ['median_dE_dt', 'mean_dissonance', 'p90_centroid', 'high_band_ratio', 
                    'low_band_ratio', 'energy_std', 'p10_centroid', 'median_flux']
    
    # 注意：Genre 列表最好基于全量数据生成，防止验证集中缺失某种风格导致维度不匹配
    genre_list = sorted(df["major_genre_name_en"].unique())
    genre_to_idx = {g: i for i, g in enumerate(genre_list)}
    
    # 使用合并后的 eval_df 创建数据集
    dataset = InferenceDataset(eval_df, MEL_FEAT_DIR, MUQ_EMB_DIR, feature_cols, len(genre_list), genre_to_idx)
    ...
    # 6. 推理阶段
    print(f"\n> 正在对 验证集+测试集 ({len(dataset)} 样本) 进行推理...")
    with torch.no_grad():
    ...
    # 调用评估函数
    p_at_k, r_at_k = evaluate_recommendation_list(all_scores_matrix, all_labels, k=K)

    print(f"\n{'='*50}")
    print(f"📊 验证集+测试集 评估结果 (K={K}):")
    print(f"   平均 Precision@{K} : {p_at_k:.4f}")
    print(f"   平均 Recall@{K}    : {r_at_k:.4f}")
    print(f"{'='*50}\n")