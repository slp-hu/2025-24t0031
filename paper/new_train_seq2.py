...
from collections import Counter  # 【新增】用于统计稀有标签
...
ES_PATIENCE = 7
BEST_MODEL_PATH = "best_ranking_model_by_ndcg2.pth" 
...
class ListNetLoss(nn.Module):
    ...
    References: Cao et al., "Learning to Rank: From Pairwise Approach to Listwise Approach", ICML 2007.
    """
    def __init__(self):
...
# ═══════════＝ 7. Experiment Execution Function (Modified for Stratification) ＝═══════════
def run_experiment(run_num, full_df, feature_cols, emb_dim, n_genres, genre_to_idx):
    print(f"\n{'='*25} STARTING RUN {run_num}/{NUM_RUNS} {'='*25}")
    
    # 【核心修改】使用我们在 main 中生成的 stratify_key 进行切分
    # 这会强制保证 训练/验证/测试 集中都有 Jazz_Romantic 的样本
    train_val_df, test_df = train_test_split(
        full_df, 
        test_size=TEST_SIZE, 
        random_state=42 + run_num, 
        stratify=full_df['stratify_key']  # <--- 使用稀有组合Key分层
    )
    ...
    train_df, val_df = train_test_split(
        ...
        random_state=42 + run_num,
        stratify=train_val_df['stratify_key'] # <--- 使用稀有组合Key分层
    )
...
# ═══════════＝ 8. Main Execution Block (Rare-Aware Stratification) ＝═══════════
if __name__ == '__main__':
    ...
    # ─────────────────────────────────────────────────────────────
    # 【核心逻辑】：构造 Rare-Aware Stratification Key
    # ─────────────────────────────────────────────────────────────
    print("> Generating Rare-Aware Stratification Keys...")
    
    # 1. 统计全局所有的 "流派_情感" 组合出现的次数
    pair_counts = Counter()
    song_pairs_map = {} # 缓存每首歌对应的 pairs

    for idx, row in df.iterrows():
        genre = str(row['major_genre_name_en'])
        emotions = str(row['emotion_sequence']).split(',') if pd.notna(row['emotion_sequence']) else []
        
        # 构造这首歌所有的 "流派_情感" 对
        pairs = [f"{genre}_{emo}" for emo in emotions]
        
        # 如果没有情感标签，就用流派本身
        if not pairs:
            pairs = [f"{genre}_None"]
            
        song_pairs_map[idx] = pairs
        pair_counts.update(pairs)

    # 2. 为每首歌分配一个 Key：选择它所有组合中“最稀有”的那个
    stratify_keys = []
    for idx, row in df.iterrows():
        pairs = song_pairs_map[idx]
        # 找到这首歌包含的组合中，全局计数最小的那个
        rarest_pair = min(pairs, key=lambda p: pair_counts[p])
        stratify_keys.append(rarest_pair)
    
    df['stratify_key'] = stratify_keys

    # 3. 处理极度稀缺样本 (防止只有 <5 个样本导致分层报错)
    key_final_counts = df['stratify_key'].value_counts()
    df['stratify_key'] = df['stratify_key'].apply(lambda x: x if key_final_counts[x] >= 5 else "Other")
    
    # 验证一下 Jazz_Romantic
    jazz_rom_count = len(df[df['stratify_key'] == 'Jazz_Romantic'])
    print(f"  -> Count of songs using 'Jazz_Romantic' as stratification key: {jazz_rom_count}")
    print("  -> These songs will now be evenly distributed across Train/Val/Test.")
    # ─────────────────────────────────────────────────────────────
