...
# ════════════════════ 配置 ════════════════════
# 必须与 Step 1 生成的文件名一致
INPUT_FILE = "top40_for_ui.npz"
FEATURE_FILE = r"C:\Users\YAO\Desktop\genre ml\paper\216\audio_features.csv" # 确保这个文件还在目录下
...
class Top40App:
    def __init__(self, root):
        self.root = root
        self.root.title("Top-40 Recommendation Explorer (Manual Eval Mode)")
        ...
        if not os.path.exists(INPUT_FILE):
            messagebox.showerror("Error", f"Run step1_export_top40.py first!")
            root.destroy()
            return
