...
# -------------------------------
# 数据集定义（目标为 arousal，包含 genre 条件）
# -------------------------------
class MusicEmotionDataset(Dataset):
    def __init__(self, csv_file):
        self.data = pd.read_csv(csv_file)

        self.features = self.data.loc[:, 'avg_pitch':'mfcc_12'].values.astype(np.float32)
        self.targets = self.data['arousal'].values.astype(np.float32).reshape(-1, 1)

        self.genre_labels = self.data['parent_genre'].astype('category')
        self.genre_ids = self.genre_labels.cat.codes.values.astype(np.int64)
        self.genre_id_to_name = dict(enumerate(self.genre_labels.cat.categories))
        self.num_genres = len(self.genre_id_to_name)
    ...
    def __getitem__(self, idx):
        ...
        genre_id = torch.tensor(self.genre_ids[idx])
        y = torch.tensor(self.targets[idx])
        return x, genre_id, y

# -------------------------------
# 模型结构（包含 genre embedding）
# -------------------------------
class emoDNN_SingleOutput(nn.Module):
    def __init__(self, input_dim, num_genres, genre_embed_dim=8, tau=4, num_hidden_layers=5, dropout_rate=0.3):
        super().__init__()
        self.genre_embed = nn.Embedding(num_genres, genre_embed_dim)
        total_input_dim = input_dim + genre_embed_dim

        hidden_dim = int(math.sqrt(total_input_dim + 1)) + tau
        layers = [nn.Linear(total_input_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout_rate)]
    ...
    def forward(self, x, genre_id):
        ...
        x_combined = torch.cat([x, genre_embed], dim=1)
...
def train(model, train_loader, val_loader, device, num_epochs=200, early_stop_enabled=True):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for x, genre_id, y in train_loader:
            x, genre_id, y = x.to(device), genre_id.to(device), y.to(device)
            optimizer.zero_grad()
            out = model(x, genre_id)
        ...
        with torch.no_grad():
            for x, genre_id, y in val_loader:
                x, genre_id, y = x.to(device), genre_id.to(device), y.to(device)
                out = model(x, genre_id)
                loss = criterion(out, y)
...
# -------------------------------
def main():
    csv_file = "Merged_Dataset.csv"  # 替换为合并后带有 genre 的 CSV
    batch_size = 20
    num_epochs = 300
