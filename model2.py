...
class AudioFeatureWithGenreDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 转换为 torch.float32 张量并添加通道维度
        feature = torch.from_numpy(feature).float().unsqueeze(0)  # 形状: (1, feature_dim)
...
class AudioGenreCNNClassifier(nn.Module):
    def __init__(self, input_dim, num_classes=13):
        super(AudioGenreCNNClassifier, self).__init__()
        
        # 第一层卷积
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm1d(16)
        self.relu = nn.ReLU()
        self.pool1 = nn.MaxPool1d(kernel_size=2)  # 输出长度: input_dim / 2
        self.dropout1 = nn.Dropout(0.4)  # Dropout 防止过拟合
        
        # 第二层卷积
        self.conv2 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm1d(32)
        self.pool2 = nn.MaxPool1d(kernel_size=2)  # 输出长度: (input_dim / 2) / 2 = input_dim / 4
        self.dropout2 = nn.Dropout(0.4)  # Dropout 防止过拟合
        
        # 计算全连接层的输入维度
        self.fc_input_dim = 32 * (input_dim // 4)
        
        self.fc1 = nn.Linear(self.fc_input_dim, 128)
        self.dropout3 = nn.Dropout(0.4)  # 全连接层 Dropout
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.pool1(x)  # (batch_size, 16, feature_dim/2)
        x = self.dropout1(x)
        ...
        x = self.bn2(x)
        x = self.relu(x)
        x = self.pool2(x)  # (batch_size, 32, feature_dim/4)
        x = self.dropout2(x)
        
        # 展平
        x = x.view(x.size(0), -1)  # (batch_size, 32 * feature_dim/4)
...
def main():
        ...
        model = AudioGenreCNNClassifier(input_dim, num_classes)
        print("模型构建成功。")

        # 训练
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"Using device: {device}")
        train(model, train_loader, val_loader, label_encoder=label_encoder, num_epochs=200, lr=1e-3, device=device)
        print("训练完成。")
