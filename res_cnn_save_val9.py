...
class ResHead(nn.Module):
    """64→32 两层 MLP + 残差（输入投影到 32 维后相加）"""
    def __init__(self, d_in: int, n_cls: int,
                 p1: float = 0.8, p2: float = 0.8):
        super().__init__()
        # 第一层：d_in → 64
        ...
        # 投影残差：d_in → 32（无偏置更简洁）
        self.proj = nn.Linear(d_in, 32, bias=False)
    ...
    def forward(self, x):
        h = self.dp1(F.relu(self.bn1(self.fc1(x))))
        h = self.dp2(self.bn2(self.fc2(h)))
        h = F.relu(h + self.proj(x))        # ← 残差相加
        return self.out(h)
