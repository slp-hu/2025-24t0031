...
# 1. 数据集定义（目标：valence）
# -------------------------------
class MusicEmotionDatasetValence(Dataset):
...
# -------------------------------
# 6. (新增) 验证集上推理并导出最优/最差案例
# -------------------------------
def inference_and_export(model, val_loader, device, out_csv="valence_val_best_worst.csv"):
    """
    在验证集上跑推理，记录每首曲子的预测值和真实值、误差；
    选出最好的 5 条和最差的 5 条记录，导出到 CSV。
    """
    model.eval()
    
    results = []
    val_index = 0
    criterion = nn.MSELoss(reduction='none')  # 用于逐样本计算 MSE
    ...
    with torch.no_grad():
        for x_batch, y_batch in val_loader:
            ...
            # 逐条计算误差
            batch_mse = criterion(preds, y_batch).cpu().numpy().flatten()
...
def main():
    ...
    # 1) 加载数据并划分训练 / 验证
    dataset = MusicEmotionDatasetValence(csv_file)
    ...
    # 4) 验证集推理并导出最优/最差 5 条记录
    inference_and_export(model, val_loader, device, out_csv="valence_val_best_worst.csv")
