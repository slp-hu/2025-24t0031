...
EMB_DIR  = "emb_full"           # MuQ 向量目录
EMB_DIM  = 1024                 # 若 768 改此
...
LR           = 5e-4
EPOCH        = 40
LR_PATIENCE  = 1     # Plateau 调 lr
ES_PATIENCE  = 5      # Early-Stopping
USE_ES       = False
BEST_PTH     = "best_muq_norm.pth"
AMB_LABELS   = ["Ambiguous"]  # 若无留空 []
...
class Net(nn.Module):
    def __init__(self, d_emb, d_gen, d_ext, n_cls):
        ...
        self.bn_ext = nn.BatchNorm1d(d_ext); self.dp_ext = nn.Dropout(0.3)
        self.head = nn.Sequential(nn.Linear(128 + 32 + d_ext, 64),
...
sched = ReduceLROnPlateau(opt, mode='min', factor=0.8, patience=LR_PATIENCE)
